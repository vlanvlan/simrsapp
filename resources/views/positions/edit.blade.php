                ...
                <form action="{{ route('positions.update', $position->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="position_level_id" name="position_level_id" required>
                            ...
                            @foreach($positionLevels as $level)
                                <option value="{{ $level->id }}" {{ old('position_level_id', $position->position_level_id) == $level->id ? 'selected' : '' }}>
                                    {{ $level->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="is_managerial" name="is_managerial" required>
                            <option value="N" {{ old('is_managerial', $position->is_managerial) == 'N' ? 'selected' : '' }}>No</option>
                            <option value="Y" {{ old('is_managerial', $position->is_managerial) == 'Y' ? 'selected' : '' }}>Yes</option>
                        </select>
                        @error('is_managerial')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
